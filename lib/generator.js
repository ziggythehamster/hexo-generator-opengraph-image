...
let blog_title_font_registered = false;
let date_font_registered = false;
let title_font_registered = false;
...
module.exports = function (locals) {
  ...
  return Promise
    ...
    .map((post) => {
      ...
      if (blog_title_font_file && blog_title_font_file_family && !blog_title_font_registered) {
        const blog_title_font_path = normalizePath(`${base_dir}/${blog_title_font_file}`);

        log.debug("Registering font '%s' (family '%s')", cyan(blog_title_font_path), magenta(blog_title_font_file_family));
        registerFont(blog_title_font_path, { family: blog_title_font_file_family });

        blog_title_font_registered = true;
      }
  
      if (date_font_file && date_font_file_family && !date_font_registered) {
        const date_font_path = normalizePath(`${base_dir}/${date_font_file}`);

        log.debug("Registering font '%s' (family '%s')", cyan(date_font_path), magenta(date_font_file_family));
        registerFont(date_font_path, { family: date_font_file_family });

        date_font_registered = true;
      }
  
      if (title_font_file && title_font_file_family && !title_font_registered) {
        const title_font_path = normalizePath(`${base_dir}/${title_font_file}`);

        log.debug("Registering font '%s' (family '%s')", cyan(title_font_path), magenta(title_font_file_family));
        registerFont(title_font_path, { family: title_font_file_family });

        title_font_registered = true;
      }
